...
void main() {
  ...
  group('Lotes por sistema (fuente: libro de la abuela)', () {
    ...
    test('musculoesqueletico completo: las 16 recetas tienen keywords', () {
      final ids = List.generate(16, (i) => 'musculoesqueletico_${(i + 1).toString().padLeft(2, '0')}');
      for (final id in ids) {
        expect(
          SearchIndex.keywordsDe(id).isNotEmpty,
          isTrue,
          reason: '$id debería tener keywords (lote musculoesqueletico completo)',
        );
      }
    });

    test('urinario completo: las 12 recetas tienen keywords', () {
      final ids = List.generate(12, (i) => 'urinario_${(i + 1).toString().padLeft(2, '0')}');
      for (final id in ids) {
        expect(
          SearchIndex.keywordsDe(id).isNotEmpty,
          isTrue,
          reason: '$id debería tener keywords (lote urinario completo)',
        );
      }
    });
